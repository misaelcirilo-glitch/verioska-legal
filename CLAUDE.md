...
### 2026-03-07: Tailwind CSS 3.4 incompatible con Next.js 16 Turbopack
- **Error**: `Module not found: Can't resolve 'fs'` — Tailwind 3.x usa `fs` internamente y Turbopack lo intenta bundlear en el cliente
- **Fix**: Actualizar a Tailwind CSS v4 + `@tailwindcss/postcss`. Cambiar `postcss.config.js` de `tailwindcss: {}` a `'@tailwindcss/postcss': {}`. Eliminar `autoprefixer` (incluido en v4)
- **Aplicar en**: Todos los proyectos con Next.js 16 + Turbopack

### 2026-03-07: Supabase IPv6 sin IPv4 — conexión directa falla desde Windows
- **Error**: `getaddrinfo ENOTFOUND` al conectar con `db.[ref].supabase.co` — solo resuelve a IPv6, Node.js en Windows no puede conectar
- **Fix**: Usar el **pooler** de Supabase (`aws-X-[region].pooler.supabase.com:6543`) o Docker local con PostgreSQL
- **Aplicar en**: Todos los proyectos que conectan directamente a Supabase DB desde Windows

### 2026-03-07: Docker Desktop requiere distribución WSL2
- **Error**: `Docker Desktop is unable to start` — WSL2 habilitado pero sin distribución instalada
- **Fix**: `wsl --install Ubuntu --no-launch` y reiniciar Docker Desktop
- **Aplicar en**: Setup inicial en Windows 11

### 2026-03-07: Next.js 16 depreca middleware.ts
- **Error**: Warning `The "middleware" file convention is deprecated. Please use "proxy" instead.`
